...
//FUNCTION TO DOWNLOAD INFO FROM POKEAPI
    func downloadPokemonDetails(completed: @escaping DownloadComplete) {
        
        //Alamofire requests the API at the URL for info, and all data is returned as "response"
                ...
                if let descrArr = dict["descriptions"] as? [Dictionary<String, String>] , descrArr.count > 0 {
                    ...
                    if let url = descrArr[0]["resource_uri"] {
                        ...
                        Alamofire.request(descrURL).responseJSON(completionHandler: { (response) in
                            ...
                            if let descrDict = response.result.value as? Dictionary<String, Any> {
                                
                                if let description = descrDict["description"] as? String {
